...
var prod = false;
var baseDir = 'debug2/';
var sql = "select RPT_REC_NUM,WKSHT_CD from HCRIS.STRONG_DATA_2013 where RPT_REC_NUM = 553807 group by RPT_REC_NUM,WKSHT_CD order by RPT_REC_NUM,WKSHT_CD";
...
connection.query(sql,function(err, rows) {
	...
	for (i = 0; i < rows.length; i++) {
        ...
        sql2 = "select RPT_REC_NUM,WKSHT_CD,LINE_NUM,CLMN_NUM,item myvalue from HCRIS.STRONG_DATA_2013 where RPT_REC_NUM like '"
                ...
                + "' and WKSHT_CD like '"
                + rows[i].WKSHT_CD
               + "' union select RPT_REC_NUM,WKSHT_CD,LINE_NUM,CLMN_NUM,item myvalue from HCRIS.STRONG_HEADERS5_2013 where RPT_REC_NUM like '"
                + rows[i].RPT_REC_NUM
                + "' and WKSHT_CD like '"
                + rows[i].WKSHT_CD
